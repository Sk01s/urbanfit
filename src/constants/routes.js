export const HOME = "/";
export const SHOP = "/shop";
export const CATEGORIES = "/category";
export const TOPS = "/category/tops";
export const BOTTOMS = "/category/bottoms";
export const HOODIES_SWEATS = "/category/hoodies-sweats";
export const JACKETS = "/category/jackets";
export const MEN = "/men";
export const SEX_CATEGORY = "/store/:sex/:category";
export const SEX = "/store/:sex";
export const COOL = "/cool/:sex";
export const LUXURY = "/luxury/:sex";
export const NEW = "/new/:sex";
export const TYPE_CATEGORY = "/type/:sex/:type";
export const WOMEN = "/women";
export const WOMEN_CATEGORIES = "/women/:category";
export const WISH = "/wish-list";
export const SETS = "/category/sets";
export const SEASONAL_PRODUCTS = "/seasonal";
export const ESSENTIAL_PRODUCTS = "/essential";
export const ACCOUNT = "/account";
export const ACCOUNT_EDIT = "/account/edit";
export const ADMIN_DASHBOARD = "/admin/dashboard";
export const ADMIN_ORDERS = "/admin/orders";
export const ORDER_DETAILS = "/admin/orders/:orderId";
export const PACKING_SLIP = "/admin/orders/:orderId/packing-slip";
export const PACKING_SLIPS_BULK = "/admin/packing-slips/pending";
export const ADMIN_PRODUCTS = "/admin/products";
export const ADMIN_USERS = "/admin/users";
export const ADMIN_IMAGES = "/admin/images";
export const ADMIN_SPECIAL_PAGES = "/admin/special-pages";
export const ADD_SPECIAL_PAGE = "/admin/special-pages/add";
export const EDIT_SPECIAL_PAGE = "/admin/special-pages/edit";
export const ADD_PRODUCT = "/admin/add";
export const EDIT_PRODUCT = "/admin/edit";
export const SEARCH = "/search/:searchKey";
export const SIGNIN = "/signin";
export const SIGNOUT = "/signout";
export const SIGNUP = "/signup";
export const FORGOT_PASSWORD = "/forgot_password";
export const CHECKOUT_STEP_1 = "/checkout/step1";
export const CHECKOUT_STEP_2 = "/checkout/step2";
export const CHECKOUT_STEP_3 = "/checkout/step3";
export const VIEW_PRODUCT = "/product/:id";
export const SPECIAL_PAGE = "/special/:id";
export const TERMS = "/terms-conditions";
export const ORDER_COMPLETED = "/order-completed/:id";
export const ORDER = "/order/:id";
export const FAQS = "/faqs";
export const ABOUT_US = "/about-us";
export const PRIVACY = "/privacy";
export const CONTACT_US = "/contact-us";
export const USER_VIEW_ORDER = "/order/:id";
export const PROMO = "/admin/promo";
export const VIEW_PROMO = "/admin/promo/:id";
export const ADD_PROMO = "/admin/promo/add";
export const ADMIN_OTP_VERIFY = "/admin/verify";
export const ADMIN_SETTINGS = "/admin/settings";
export const ADMIN_CATEGORIES = "/admin/categories";
export const ADMIN_TYPES = "/admin/types";
export const ADMIN_PROMO_POPUP = "/admin/promo-popup";
