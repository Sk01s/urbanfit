...
// Import services
import firebase from "./src/services/firebaseAdmin.js";
import backblazeB2 from "./src/services/backblazeB2.js";
...
fastify.post("/api/email/wishlist-reminder", async (request, reply) => {
  try {
    ...
    const result = await emailService.sendWishlistReminder(userEmail, userName, product);
...
fastify.get("/api/admin/verification-status/:adminId", async (request, reply) => {
  try {
...
async function getWishlistEmailSettings() {
  try {
    const doc = await firebase.db.collection("settings").doc("wishlistEmail").get();
    if (doc.exists) {
      return doc.data();
    }
...
fastify.post("/api/wishlist/schedule-reminder", async (request, reply) => {
  try {
    ...
    const sendAt = (addedAt || Date.now()) + settings.delayHours * 60 * 60 * 1000;
...
fastify.delete("/api/wishlist/cancel-reminder/:userId/:productId", async (request, reply) => {
  try {
...
fastify.put("/api/admin/settings/wishlist-email", async (request, reply) => {
  try {
    ...
    await firebase.db.collection("settings").doc("wishlistEmail").set(updates, { merge: true });
...
cron.schedule("0 * * * *", async () => {
  ...
  for (const [key, item] of wishlistEmailQueue.entries()) {
    if (now >= item.sendAt) {
      try {
        await emailService.sendWishlistReminder(item.userEmail, item.userName, item.product);
        wishlistEmailQueue.delete(key);
        sent++;
        console.log(`Sent wishlist reminder to ${item.userEmail} for product ${item.product.name}`);
      } catch (error) {
