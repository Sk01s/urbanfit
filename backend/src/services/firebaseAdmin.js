...
class FirebaseAdmin {
  constructor() {
    if (!admin.apps.length) {
      ...
      try {
        // Try loading from service-account.json in backend root
        const serviceAccountPath = join(__dirname, "../service-account.json");
        console.log(
          `Attempting to load Firebase service account from ${serviceAccountPath}`
        );
    ...
    const visit = (value, path) => {
      if (typeof value === "string") {
        if (this.isFirebaseStorageUrl(value)) {
          storageUrls.push({ path: path.join("."), url: value, type: "firebase" });
        } else if (this.isBackblazeB2Url(value)) {
          storageUrls.push({ path: path.join("."), url: value, type: "backblaze" });
        }
        return;
      }
